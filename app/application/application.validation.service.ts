module Application {
    ...
    export interface IValidationService {
        checkIsAFM(afm: string): boolean;
        checkIsAMKA(amka: string): boolean;
        checkIsAcademicId(academicId: string): boolean;
    }

    export class ValidationService implements IValidationService {
        checkIsAFM(afm): boolean {
            if (!afm || afm.length == 0)
            {
                return true;
            }
    ...
    ApplicationModule.service('ApplicationValidationService', ValidationService);
}